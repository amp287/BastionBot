...
public class MusicQueue{
	...
	public int AddToQueue(String arg, ArrayList<String> options){
		...
		if(locked) return -1;
		System.out.println("AddToQueue():Lock!");
		locked = true;
	...
	public int DeleteFromQueue(){
		MusicTrack track;
		if(locked) return -1;
    System.out.println("DeleteFromQueue(): Lock!");
		locked = true;
...
//gets a downloaded track that is ready to be played
	public MusicTrack GetReadyTrack(){
		...
		System.out.println("GetReadyTrack():Lock!");
		locked = true;
	...
	public String PrintQueue(){
		...
		if(locked) return null;
		System.out.println("PrintQueue():Lock!");
		locked = true;
	...
	public MusicTrack GetNextDownload(){
		MusicTrack track;
		if(locked)return null;
		System.out.println("GetNextDownload():Lock!");
		locked = true;
	...
	public void FinishDownloadNotify(MusicTrack result){
		...
		locked = true;
		System.out.println("FinishDownloadNotify():Lock!");
		...
		for(int i = 0; i < queue.size(); i++){
			if((track = queue.get(i)).id == result.id){
				...
				System.out.println("FinishDownloadNotify():queue val:" + queue.get(i).downloaded + " track:" + track.downloaded + " result:" + result.downloaded);
				break;
			}
		}
		locked = false;
